...
const PublicLanding = () => {
  ...
  useLayoutEffect(() => {
		...
		if (establishment.body_background_type === "gradient") {
			return {
				background: `linear-gradient(${
					establishment.body_gradient_angle || 45
				}deg, ${establishment.body_gradient_color1 || "#3B82F6"}, ${
					establishment.body_gradient_color2 || "#8B5CF6"
				})`,
			};
		}
		return {
	...
	const getHeaderStyles = () => {
		...
		if (establishment.header_background_type === "gradient") {
			styles.background = `linear-gradient(${
				establishment.header_gradient_angle || 45
			}deg, ${establishment.header_gradient_color1 || "#3B82F6"}, ${
				establishment.header_gradient_color2 || "#8B5CF6"
			})`;
		} else {
	...
	const getServicesStyles = () => {
		if (
			establishment.services_background_type === "image" &&
			establishment.services_background_image
		) {
			return {
	...
	return (
		<div
			...
			style={{ ...getBodyStyles(), ["--primary-color" as any]: primaryColor }}
		>
			{/* Header */}
			<header
				ref={headerRef}
				className={`border-b ${
					establishment.header_position === "fixed"
			...
			<section
				...
				style={{
					marginTop:
						establishment.header_position === "fixed" ? headerHeight : undefined,
					backgroundColor: establishment.hero_background_color || primaryColor,
					backgroundImage: establishment.hero_image_url
						? `url(${establishment.hero_image_url})`
						: undefined,
					backgroundSize: "cover",
					backgroundPosition: "center",
				}}
			>
				...
				<div className="container mx-auto relative z-10">
					<h1
						className={`text-${establishment.hero_title_font_size || "4xl"} md:text-${
							establishment.hero_title_font_size || "6xl"
						} font-bold mb-6`}
					...
					<p className="text-xl md:text-2xl mb-8 max-w-3xl mx-auto">
						{establishment.hero_description ||
			...
			{services.length > 0 && (
				<section className="py-16 px-4" style={getServicesStyles()}>
					<div className="container mx-auto">
						<h2
							className={`text-${
								establishment.services_title_font_size || "3xl"
							} font-bold text-center mb-12`}
							style={{
								fontFamily: establishment.services_title_font_family || "inherit",
							}}
						>
							Nossos Serviços
						</h2>
			...
			<section className="py-16 px-4 bg-muted/30">
				...
				{establishment.section2_enabled && establishment.section2_content && (
					<div
						dangerouslySetInnerHTML={{ __html: establishment.section2_content }}
					/>
				)}
			</section>
			...
			<footer
				...
				style={{
					backgroundColor: establishment.footer_background_color || "#f8f9fa",
				}}
			>
				<div className="container mx-auto">
					<p
						className={`text-${establishment.footer_font_size || "sm"} text-${
							establishment.footer_text_align || "center"
						}`}
